...
void AudioPrintMenu()
{
	...
	if (recordingValid){
		...
		xil_printf("5 - Print recorded data\n\r");
	}
	xil_printf("q - Quit\n\r");
	xil_printf("\n\r");
...
int AudioRunDemo(u32 i2sAddr, u32 uartAddr)
{
	char userInput = 0;
	...
	while (userInput != 'q')
	{
		...
		case '1':
		  ...
		  AudioPlayTone(i2sAddr, uartAddr);
			break;
		case '2':
			AudioRec(i2sAddr, 0);
			break;
...
int AudioPlayTone(u32 i2sAddr, u32 uartAddr)
{
	u32 userInput = 0;
	u32 btnState = 0;
	u32 swState = 0;
...
//			btnState = Xil_In32(btnAddr);
      btnState = buttons_read();
//			swState = Xil_In32(swAddr);
      swState = switches_read();
...
int AudioSaveData(u32 i2sAddr){
    ...
    int index = 0;
        ...
        xil_printf("u32 shootSoundData[REC_SAMPLES] = {");
        for (index = 0; index < REC_SAMPLES ; index++)
        {
            xil_printf("%d, ", recDataL[index]);
        }
...
int AudioPlayRec(u32 i2sAddr)
{
	...
	for (index = 0; index < REC_SAMPLES ; index++)
	{
	    ...
	    //I2SFifoWrite(i2sAddr,shootSoundData[index]);
	    //I2SFifoWrite(i2sAddr,shootSoundData[index]);
	}
